...
public class OneFuelWeight implements E6BCalculation
{
    ...
    public OneFuelWeight(String name, String desc, double d)
    {
        fName = name;
        fDesc = desc;
        fLbsPerGal = d;
        
        fInValue = new Value(Volume.VOLUME_GALLONS);
        fOutUnit = Weight.WEIGHT_LBS;
    }
    ...
    public void calculate(List<CalculatorInputView> in, List<CalculatorInputView> out, boolean store)
    {
        ...
        double a = vin.getValueAsUnit(Volume.VOLUME_GALLONS);
        double v = a * fLbsPerGal;
        vout.setValue(v, Weight.WEIGHT_LBS);
