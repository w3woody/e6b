...
public class ReqTrueAirspeed implements E6BCalculation
{
    ...
    public void calculatorInitialize(List<CalculatorInputView> in, List<CalculatorInputView> out)
    {
        ...
        d.loadStoreValue(CalcStorage.shared().groundSpeed);

        a = out.get(0);
        b = out.get(1);
        a.setUnit(0);
        b.setUnit(CalcStorage.shared().trueAirSpeed.getUnit());
    }
    ...
    public void calculate(List<CalculatorInputView> in, List<CalculatorInputView> out, boolean store)
    {
        ...
        if (store) {
            ...
            CalcStorage.shared().groundSpeed = d.saveStoreValue();
        }
        ...
        double ax = Math.sin(cv) * dv - Math.sin(av) * bv;
        double ay = Math.cos(cv) * dv - Math.cos(av) * bv; /* gv - wv = cv */
